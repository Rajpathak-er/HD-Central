<?php /* Template Name: Ebay CSV Template */ 

//8183
//$_POST['pending_vendor'] = 'true';
//do_action('user_register',8183);
//
//die;
...
$fields = ebay_csv_header();
   ...
   // $ra_random_index = 1;
	...
	echo $ebay_category = $region_array[$ra_random_index]['ebay_category'];
...
//$pdata = get_product_details('194987791582');
//print_r($pdata );
//die;
...
// https://svcs.ebay.com/services/search/FindingService/v1?OPERATION-NAME=findItemsByCategory&SERVICE-VERSION=1.0.0&SECURITY-APPNAME=JohannWi-HDCentra-PRD-ac8eaa0db-6cade1e5&RESPONSE-DATA-FORMAT=JSON&REST-PAYLOAD&keywords='.rawurlencode($keywords).'&GLOBAL-ID='.$country_code.'&categoryId='.$ebay
...
echo "count: ".count($obj->findItemsAdvancedResponse[0]->searchResult[0]->item)."<br>";	
$singleData = array();	
$i = 0;	
...
//echo "</pre>";
	...
	$attrArr["PictureURL"] = array($bikedata->pictureURLSuperSize[0]);
...
foreach ($newdatatable as $key => $item) {
	...
	echo $post_id = get_post_id_by_meta_key_and_value('ItemId',$item['ItemId']);
...
echo "post_id".$post_id;
		...
		update_post_meta( $post_id, 'ItemId', $item['ItemId']); 	
		...
		update_post_meta( $post_id, 'engine', $item['Engine Size']); 	
		update_post_meta( $post_id, 'stock_number', $item['ItemId']); 	
		update_post_meta( $post_id, 'title', 'hide'); 	
		update_post_meta( $post_id, 'breadcrumbs', 'show'); 	
		update_post_meta( $post_id, 'gallery_video', ''); 	
		//update_post_meta( $post_id, 'vin_number', $item['Date of 1st Registration']); 	
		update_post_meta( $post_id, 'registration_date', $item['Date of 1st Registration']); 	
		update_post_meta( $post_id, 'stm_car_location', $location); 	
		update_post_meta( $post_id, 'ca-year', $item['Year']); 	
		update_post_meta( $post_id, 'drive', $item['Drive Type']); 	
		update_post_meta( $post_id, 'exterior-color', $item['Colour']); 	
		update_post_meta( $post_id, 'ListingType', $item['ListingType']); 	
		
		if($item['ConditionDisplayName'] == 'Used'){
			$condition ='used-cars';
			update_post_meta( $post_id, 'condition', 'used-cars'); 	
		}else{
			update_post_meta( $post_id, 'condition', 'new-cars'); 	
		}

		update_post_meta( $post_id, 'no-of-owners', $item['Previous owners (excl. current)'] + 1); 	
		update_post_meta( $post_id, 'make', $item['Model']); 	
		if(!empty($item['MOT Expiration Date'])){
			update_post_meta( $post_id, 'mot', 'yes'); 	
		}
	


		update_post_meta( $post_id, 'history_link', ''); 	
		update_post_meta( $post_id, 'stm_lat_car_admin', $latlongarray[0]); 	
		update_post_meta( $post_id, 'stm_lng_car_admin', $latlongarray[1]); 	
		update_post_meta( $post_id, 'additional_features', ''); 	
		update_post_meta( $post_id, 'stm_additional_features', ''); 	
		update_post_meta( $post_id, '_thumbnail_id', ''); 	
		update_post_meta( $post_id, 'property_featured_image', $item['Gallery URL']); 
		update_post_meta( $post_id, 'URL', $item['URL']); 
		update_post_meta( $post_id, 'stm_car_user', 1360); 
		
		update_post_meta( $post_id, 'gallery',  $item['PictureURL']); 	
        update_post_meta( $post_id, 'gallery_ebay',  $item['PictureURL']);   
		...
		if(!empty($item['Model'])){
			...
			$term = term_exists( $destination, 'make' );
            var_dump($term);
				...
				if (!empty($term)) {
                    ...
                    $term = get_term( $term_id, 'make' );
                    $slug = $term->slug;
                    $tag = array( $term_id); 
                    
                    wp_set_post_terms( $post_id, $tag, 'make' );
                    update_post_meta( $post_id, 'make', $slug);   

				}else{
                    echo 'uncategorized 2';
					/*$term =  wp_insert_term(
                ...
                wp_set_post_terms( $post_id, $tag, 'make' );
                update_post_meta( $post_id, 'make', 'uncategorized');   					
		...
		if(!empty($item['Year'])){
			$destination = $item['Year'];
			$term = term_exists( $destination, 'ca-year' );
			...
			if($destination != ""){
				...
				$term_id = $term['term_id']; 
				$tag = array( $term_id); 
                update_post_meta( $post_id, 'ca-year', $destination);   
				
				wp_set_post_terms( $post_id, $tag, 'ca-year' );
			}
		}

			$countryname = code_to_country($item['Country']);
			echo "<br/>".$countryname;
            
			$countrynameterm = term_exists( $countryname, 'region' );
			...
			$term = term_exists( $destination, 'condition' );
			...
			if($destination != ""){
				...
				wp_set_post_terms( $post_id, $tag, 'condition' );
			}
		
			if($item['ListingType'] == 'Auction'){
				$tag = array('1895'); 
				wp_set_post_terms( $post_id, $tag, 'seller' );
                update_post_meta( $post_id, 'seller', 'ebay-listing-auction');   

			}else{
				$tag = array('1894'); 
				wp_set_post_terms( $post_id, $tag, 'seller' );
                update_post_meta( $post_id, 'seller', 'ebay-listing-for-sale');   
			}
