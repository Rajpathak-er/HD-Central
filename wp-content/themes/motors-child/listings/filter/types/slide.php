...
<div class="col-md-12 col-sm-12 123 stm-filter_<?php echo $slug; ?>">
    <div class="filter-<?php echo esc_attr($vars['slug']); ?> stm-slider-filter-type-unit">
        <div class="clearfix">
            <h5 class="pull-left cstm_cls">
				<?php if($vars['slug']=='price'){ ?>
					<div class="imginv-icon"><img src="<?php echo get_home_url(); ?>/wp-content/uploads/2021/09/preferences-2.png" ></div>
				<?php } ?>
                    <?php 
                    if($vars['slug'] == 'price' || $vars['slug'] == 'mileage' ) {
                        echo "Max ";
                    }
                        ?>
                <?php _e($taxonomy['single_name'], 'motors'); ?></h5>
            ...
            <div class="col-md-12 col-sm-12">
                <?php 
                    if($vars['slug'] == 'price') {
                        ...
                        ?>
            <div class="stm-current-slider-labels" style="display: inline;"><?php echo esc_html($vars['label']); ?></div>
                <input type="text" style="display: none;" name="max_<?php echo esc_attr($vars['slug']); ?>"
                       id="stm_slide_filter_max_<?php echo esc_attr($vars['slug']); ?>" class="form-control"/>
                        <?php 
                    }
                
                    if($vars['slug'] == 'mileage') {

                        ?>
            <div class="stm-current-slider-labels"  style="display: inline;"><?php echo esc_html($vars['label']); ?></div> miles
                <input type="text" style="display: none;" name="max_<?php echo esc_attr($vars['slug']); ?>"
                       id="stm_slide_filter_max_<?php echo esc_attr($vars['slug']); ?>" class="form-control"/>
                        <?php 
