...
<head>
    ...
    ?>
    <style type="text/css">
    <?php $top_image_bar = get_field('top_image_bar'); 
if(!empty($top_image_bar)){


    ?>
   
body.page .entry-header {
    background-image: url("<?php echo $top_image_bar; ?>") !important;
}
...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-172134326-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-172134326-1');
</script>
...
?>
        <?php if ( !stm_is_auto_parts() ) { ?>
            ...
            <?php if ( !is_404() and !is_page_template( 'coming-soon.php' ) ) { ?>
                ...
                <div id="header" class="whichheadertofind">
                    <?php
                    if ( is_listing( array( 'listing', 'listing_two', 'listing_three' ) ) ) {
                        ...
                        get_template_part( 'partials/header/header-equipment' );
                    } else {
                        get_template_part( 'partials/header/header' );
                        get_template_part( 'partials/header/header-nav-new-1' );
                    }
                    ?>
                </div> <!-- id header -->
