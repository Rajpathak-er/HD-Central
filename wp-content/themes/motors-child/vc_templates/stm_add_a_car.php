...
<div class="stm_add_car_form <?php echo esc_attr($stm_edit_car_form); echo esc_attr($css_class); ?>">
    ...
    <form method="POST" action="" enctype="multipart/form-data" id="stm_sell_a_car_form">
        ...
        if( class_exists('Subscriptio') || class_exists('RP_SUB')) stm_listings_load_template('add_car/step_7', $vars); ?>
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function(){
        jQuery('#findbikedetails').click(function(){
            var regdata = jQuery("#regdata").val(); 
            ...
            jQuery.ajax({       
                ...
                success:function(response){
                    ...
                    $('input[name ="stm_s_s_engine"]').val(response.VehicleDetails.CylinderCapacity);
                    $('input[name ="stm_s_s_engine"]').prop('readonly', true);

                    $('#years').val(response.VehicleDetails.Year); // Select the option with a value of '1'
                    $('#years').trigger('change');
                    //$('#years').select2({disabled:'readonly'});

                    if(response.VehicleMotData.length>0){
                        $('input[name ="stm_s_s_mileage"]').val(response.VehicleMotData[response.VehicleMotData.length-1].Odometer);
                        $('input[name ="stm_s_s_mileage"]').prop('readonly', true);
                        $('select[name ="stm_s_s_mot"]').val("yes"); // Select the option with a value of '1'
                        $('select[name ="stm_s_s_mot"]').trigger('change');
                      //  $('select[name ="stm_s_s_mot"]').select2({disabled:'readonly'});
                    }else{
                        $('select[name ="stm_s_s_mot"]').val("no"); // Select the option with a value of '1'
                        //$('select[name ="stm_s_s_mot"]').trigger('change');
                    }
                    $('#stm_s_s_exterior-color').val(response.VehicleDetails.Colour); // Select the option with a value of '1'

                    $('#stm_s_s_exterior-color').trigger('change');
                    var d = new Date(response.VehicleDetails.DateOfRegistration);
                    $('.stm_registration_date input').val(GetFormattedDate(d));
                    $('.stm_registration_date input').prop('readonly', true);
                    $('.stm_registration_date input').datetimepicker('destroy');
                    $('input[name="stm_vin"]').val(response.VehicleDetails.Registration); 
                    $('input[name="stm_vin"]').prop('readonly', true);
                    $('.text_stm_model_ref').val(response.VehicleDetails.Model);
                    $('.text_stm_model_ref').prop('readonly', true);
                    ...
                    //response.VehicleDetails.CylinderCapacity
