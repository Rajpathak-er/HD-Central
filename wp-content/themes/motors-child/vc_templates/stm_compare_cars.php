...
$add_to_text = (stm_is_aircrafts()) ? esc_html__('Add to compare', 'motors') : esc_html__('Add bike to compare', 'motors');
...
$title_text = esc_html__('Compare Bikes', 'motors');
if(stm_is_boats()) {
...
<div class="<?php echo esc_attr($css_class); ?>">
    ...
    <?php if (!empty($compare_ids) or count($compare_ids) != 0): ?>
        ...
        if ($compares->have_posts()): ?>
            <div class="row row-4 car-listing-row stm-car-compare-row rrreerr">
                        ...
                        <a href="<?php the_permalink(); ?>" class="rmv_txt_drctn">
                            <div class="compare-col-stm-empty">
                                <div class="image">
                                    ...
                                    $ItemId = get_post_meta($post_id, 'ItemId',true);
                                    if($ItemId){ ?>
                                         <div class="stm-compare-car-img">
                                             <img class="stm-compare-empty"
                                             src="<?php echo $car_media_images[0]; ?>"
                                             alt="<?php esc_attr_e('', 'motors'); ?>"/>
        ...
        <?php if ($compares->have_posts()): 
            $newfilteroption = array('slug' =>'location','single_name' => 'Location');
                            //$filter_options[]= $newfilteroption;        
                            array_unshift($filter_options , $newfilteroption);
                            ...
                            <div class="compare-values">
                                ...
                                <table>
                                    ...
                                    <?php foreach ($filter_options as $filter_option): ?>
                                        <?php if($filter_option['slug'] == 'location'){
                                        ...
                                        } else if ($filter_option['slug'] != 'price') { ?>
                                            <tr>
                                                <?php $compare_option = get_post_meta(get_the_id(), $filter_option['slug'], true); ?>
                                                <td class="compare-value-hover <?php echo esc_attr('compare-value-' . $filter_option['slug']) ?>"
                                                    data-value="<?php echo esc_attr('compare-value-' . $filter_option['slug']) ?>">
