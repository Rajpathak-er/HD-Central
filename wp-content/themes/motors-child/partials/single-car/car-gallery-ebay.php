...
<div class="stm-car-carousels">
	...
	<div class="stm-big-car-gallery">
		...
		<?php if($car_media_images):
			$full_src = $car_media_images[0];

			//Post thumbnail first ?>
			<div class="stm-single-image" data-id="big-image-<?php echo esc_attr(get_post_thumbnail_id($post_id)); ?>">
				<a href="<?php echo esc_url($full_src); ?>" class="stm_fancybox" rel="stm-car-gallery">
					<img src="<?php echo esc_url($full_src); ?>" alt="<?php printf(esc_attr__('%s full','motors'), get_the_title($post_id)); ?>"/>
		...
		<?php if(!empty($car_media_images)): ?>
			<?php foreach ( $car_media_images as $gallery_image ): ?>
				<?php $src  = $gallery_image; ?>
				<?php $full_src  = $gallery_image;  ?>
				<?php if(!empty($src) && $gallery_image != $car_media_images[0]): ?>
					<div class="stm-single-image" data-id="big-image-<?php echo esc_attr($gallery_image); ?>">
						<a href="<?php echo esc_url($full_src); ?>" class="stm_fancybox" rel="stm-car-gallery">
							<img src="<?php echo esc_url($src); ?>" alt="<?php printf(esc_attr__('%s full','motors'), get_the_title($post_id)); ?>"/>
						</a>
					</div>
				<?php endif; ?>
			<?php endforeach; ?>
	...
	<?php if($car_media_images[0] || ( !empty($video_preview) and !empty($gallery_video) ) ): ?>
		<div class="stm-thumbs-car-gallery">

			<?php if($car_media_images[0]):
				//Post thumbnail first ?>
				<div class="stm-single-image" id="big-image-<?php echo $car_media_images[0]; ?>">
					
					<img src="<?php echo esc_url($car_media_images[0]); ?>" alt="<?php printf(esc_attr__('%s full','motors'), get_the_title($post_id)); ?>"/>
				</div>
			<?php endif; ?>
