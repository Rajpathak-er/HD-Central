...
if(is_singular('manuals')){
	$title =  url_title($title);

//	$title = url_title($title). '<span style="float:right">'.wpfp_link().'<span>';
}
    ...
    //$title_tag                           = (empty(get_post_meta( $post_id, 'stm_title_tag', true ))) ? 'h1' : get_post_meta( $post_id, 'stm_title_tag', true ); 
    $title_tag                           =  'h1' ; 
...
if ( $breadcrumbs != 'hide' ):
	...
	if ( $is_shop || $is_product || $is_product_category ) {
		...
		if ( function_exists( 'bcn_display' ) ) { ?>
			<div class=" breadcrumbs_custom heading-font <?php echo esc_attr($blog_margin); ?>">
				<div class="container">
    ...
    <div class="entry-header <?php echo esc_attr($alignment.$additional_classes.$disable_overlay); ?>" style="<?php echo stm_do_lmth($title_style); ?>">
	    <div class="container">
            <div class="entry-title vc_row">
                ...
                <<?php echo esc_attr($title_tag); ?> class="h1 3333" style="text-align: left; <?php echo implode( ' ', $title_style_h1 ); ?>">
	                <?php
		                if(!empty($sub_title_instead) and stm_is_motorcycle()) {
			                ...
			                if(is_singular('manuals')){
								//	$title =  url_title($title);
									...
									echo '<span style="float:right">'.wpfp_link().'<span>';
							}
		                }
		            ?>
...
<style type="text/css">
	...
	.post_title_main_heading .wpfp-span{
		    float: right;
