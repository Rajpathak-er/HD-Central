		...
		<div class="image">
			...
			<a href="<?php the_permalink() ?>" class="rmv_txt_drctn">
				<div class="image-inner">
					...
					<?php else : ?>
						...
						/>-->
						<img
							src="<?php echo $upload_dir['baseurl'].'/2020/07/noimage.png'; ?>"
							class="img-responsive"
							alt="<?php esc_attr_e('Placeholder', 'motors'); ?>"
						/>
					<?php endif; ?>
			...
			<?php if(get_post_status(get_the_id()) != 'pending'): ?>
				<div class="stm_edit_disable_car heading-font">
					...
					<?php if($asSold == 'on'): ?>
						<a href="<?php echo esc_url(add_query_arg(array('stm_unmark_as_sold_car' => get_the_ID(),'page' => 'inventory'), stm_get_author_link(''))); ?>" class="as_sold">
							<?php esc_html_e('Unmark as sold', 'motors'); ?><i class="fa fa-check-square-o" aria-hidden="true"></i>
						</a>
					<?php else : ?>
						<a href="<?php echo esc_url(add_query_arg(array('stm_mark_as_sold_car' => get_the_ID(),'page' => 'inventory'), stm_get_author_link(''))); ?>">
                    ...
                    <?php /* ?><a href="<?php echo stm_get_author_link('')."?page=add_bike&edit_car=1&item_id=".get_the_ID(); //esc_url(stm_get_add_page_url('edit', get_the_ID())); ?>"
                       data-toggle="tooltip"
                       data-placement="top"
					...
					<?php if(get_post_status(get_the_id()) == 'draft'): ?>
						<a href="<?php echo esc_url(add_query_arg(array('stm_enable_user_car' => get_the_ID(),'page' => 'inventory'), stm_get_author_link(''))); ?>" class="enable_list"
                           data-toggle="tooltip"
                           data-placement="top"
                           title="<?php esc_html_e('Enable', 'motors') ?>"
                        ><?php esc_html_e('Enable', 'motors'); ?><i class="fa fa-eye"></i></a>
					<?php else: ?>
						<a href="<?php echo esc_url(add_query_arg(array('stm_disable_user_car' => get_the_ID(),'page' => 'inventory'), stm_get_author_link(''))); ?>" class="disable_list"
                           data-id="<?php esc_attr(get_the_ID()); ?>"
                           data-toggle="tooltip"
                           data-placement="top"
                           title="<?php esc_html_e('Disable', 'motors') ?>"
                        ><?php esc_html_e('Disable', 'motors'); ?><i class="fa fa-eye-slash"></i></a>
					<?php endif; ?>
					<?php
					if(get_theme_mod('dealer_payments_for_featured_listing', false)) :?>
							...
							if(!$special_car && (empty($featuredStatus) || $featuredStatus == 'in_cart')) :
                                ...
                                >
                                    <?php esc_html_e('Favorite', 'motors') ?><i class="fa fa-star" aria-hidden="true"></i>
                                </a>
							<?php else :
			...
			<div class="stm_edit_disable_car">
				<?php if(get_post_status(get_the_id()) == 'draft'): ?>
					<div class="">
						<a class="stm-delete-confirmation" href="<?php echo esc_url(add_query_arg(array('stm_move_trash_car' => get_the_ID()), stm_get_author_link(''))); ?>" data-title="<?php the_title(); ?>">
						<?php esc_html_e('Draft', 'motors'); ?>
							<i class="fa fa-trash-o"></i>
						</a>
					</div>
				<?php endif; ?>
