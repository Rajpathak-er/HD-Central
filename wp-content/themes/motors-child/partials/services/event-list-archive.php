...
//print_r($allregion);
	...
	$event_additional_details = get_terms( array(
		'taxonomy' => 'event_additional_detail',
		'hide_empty' => false,
	) );	
...
$radius = (!empty(get_theme_mod("distance_search", ""))) ? get_theme_mod('distance_search', '') : 100;
 ...
 $curdate = date('Y-m-d');
			...
			if( empty($_GET['ca_location']) && empty($_GET['all_location'] )){
		...
		//	echo "+++".count($event_query->posts)."<br>";
			...
			if ( ! empty( $event_query->posts) ) {				
				foreach ( $event_query->posts as $none_dealer_user ) {				
					...
					$userlat =  get_post_meta($none_dealer_user->ID,'lat',true);
					//echo "<br/>";
					$userlng =  get_post_meta($none_dealer_user->ID,'lng',true);
					//echo "<br/>";
					$location_address = get_field('location_address', $none_dealer_user->ID);
					//$userlat = $userlng = '';
					//print_r($location_address);
					if(!empty( $location_address )){
					//	echo "dfdfdfdf";
						$userlat = $location_address['lat'];
						$userlng = $location_address['lng'];
						update_post_meta($none_dealer_user->ID,'lat',$userlat);
						update_post_meta($none_dealer_user->ID,'lng',$userlng);
					}


					
					//echo "****".$none_dealer_user->ID."+++++".$userlat." ----".$userlng."<br>";
			...
			foreach ($none_dealer_users as $key => $none_dealer_user) {
				...
				$event_lng = $geot_options['radius_lng'];
							...
							$location_address = get_field('location_address', $none_dealer_user->ID);
							if( $location_address ){
								$event_lat = $location_address['lat'];
								$event_lng = $location_address['lng'];
							}
    ...
    <div class="col-md-3 col-sm-12 classic-filter-row sidebar-sm-mg-bt <?php echo esc_attr($sidebar_pos['sidebar']); ?>" style="margin-top: -20px;">
		...
		<div class="count_list_n">
			...
			$total_count_stm_u = count(get_posts( $argsnew ));	
				...
				<div class="col-xs-3 search-div"><span class="text_after_counter">Events</span></div>
			</div>
		</div>
		...
		<div class="stm-inventory-sidebar dealerships-sidebar">
			...
			<div class="stm-accordion-single-unit stm-listing-directory-checkboxes stm-one_col stm-ajax-checkbox-instant event-category">
				<a class="title <?php echo $dealercategory; ?>" data-toggle="collapse" href="#accordion-dealerships-category" aria-expanded="false">
					<h5><i class="far fa-flag"></i> Event Categories</h5>
			...
			<div class="stm-accordion-single-unit stm-listing-directory-checkboxes stm-one_col stm-ajax-checkbox-instant event-detail">
		...
		<br/>
			...
			<?php  echo do_shortcode('[widget id="custom_html-10"]'); ?>
        ...
        <div id="listings-result">
          ...
          //  echo "ffdfdkfdfdf";
			...
			foreach ($events as $key => $event) {
				...
				include("content/event-listing.php");
