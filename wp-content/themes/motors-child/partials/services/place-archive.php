    ...
    <div class="col-md-3 col-sm-12 classic-filter-row sidebar-sm-mg-bt <?php echo esc_attr($sidebar_pos['sidebar']); ?>" style="margin-top: -20px;">
		...
		if(!empty($_GET['things'])){                
				...
				foreach( $things as $thing ){
					$args['meta_query'][] = array(
						'key'     => 'things_to_do',
						'value' => sprintf(':"%s";', trim($thing)),
						'compare' => 'LIKE'
					);
				}				
            }	
			
			if(!empty($_GET['place_category'])){                
				$place_category = $_GET['place_category'];				
				foreach( $place_category as $cat ){
					$args['meta_query'][] = array(
						'key'     => 'place_categories',
						'value' => sprintf(':"%s";', trim($cat)),
						'compare' => 'LIKE'
					);
				}				
            }else{
			...
			if( empty($_GET['ca_location']) && empty($_GET['all_location'] ) && 1==2){
        ...
        //echo "+++".count($none_dealer_users_prelist)."<br>";
			...
			$total_count_stm_u = count(get_users( $args ));
				...
				<div class="col-xs-3 search-div"><span class="text_after_counter">Places</span></div>
			</div>
		</div>
<form  method="get" action="/places/">   
        ...
        <div class="stm-inventory-sidebar dealerships-sidebar 76867">
...
<div class="stm-accordion-single-unit stm-listing-directory-checkboxes stm-one_col stm-ajax-checkbox-instant">
						<a class="title <?php echo $dealercategory; ?>" data-toggle="collapse" href="#accordion-places-category" aria-expanded="false">
							<h5><div class="imginv-icon"><i class="far fa-flag region"></i></div> Place Categories</h5>
							<span class="minus"></span>
						</a>
						<div class="stm-accordion-content">
							<div class="collapse content <?php echo $dealercategory2;?>" id="accordion-places-category">
            	 ...
            	 <div class="stm-keyword-search-unit" style="clear: both;">
                            <?php foreach($allplaces as $place) {?>
                              <label class="stm-option-label" style="display: block;">
                                <input type="checkbox" class="chk_filter_removeall chk_filter_condition" data-class="chk_all_filter_condition"  name="place_category[]"
                                value="<?php echo $place->term_id; ?>" <?php if( !empty($_GET['place_category']) && in_array($place->term_id, $_GET['place_category']) ){ echo " checked=checked "; } ?> />
...
<div class="stm-accordion-single-unit stm-listing-directory-checkboxes stm-one_col stm-ajax-checkbox-instant">
						<a class="title <?php echo $dealercategory; ?>" data-toggle="collapse" href="#accordion-things-category" aria-expanded="false">
							<h5><div class="imginv-icon"><img src="<?php echo get_home_url(); ?>/wp-content/uploads/2021/09/preferences-2.png" class="model_range"></div> Things To Do</h5>
							<span class="minus"></span>
						</a>
						<div class="stm-accordion-content">
							<div class="collapse content <?php echo $dealercategory2;?>" id="accordion-things-category">
								<div class="stm-accordion-content-wrapper stm-accordion-content-padded">
									<div class="stm-accordion-inner">

            
            	 <div class="stm-keyword-search-unit" style="clear: both;">
                            <?php foreach($allthings as $thing) {?>
                              <label class="stm-option-label" style="display: block;">
                                <input type="checkbox" class="chk_filter_removeall chk_filter_condition" data-class="chk_all_filter_condition"  name="things[]"
                                value="<?php echo $thing->term_id; ?>" <?php if( !empty($_GET['things']) && in_array($thing->term_id, $_GET['things']) ){ echo " checked=checked "; } ?> />
                                <span><?php echo $thing->name;?></span>
                            </label>
		...
		<div class="stm-accordion-single-unit stm-listing-directory-checkboxes stm-one_col stm-ajax-checkbox-instant" style="display: none;">
		...
		<br/>
			...
			<?php  echo do_shortcode('[widget id="custom_html-9"]'); ?>
...
<!--
        ...
        //echo "+++".count($none_dealer_users_prelist)."<br>";
			...
			//print_r($none_dealer_users);

            if(count($none_dealer_users) == 0){
                echo  "No Place Subscriber found";
            }
            foreach ($none_dealer_users as $key => $none_dealer_user) {
# code...
                $userimg = get_user_meta($none_dealer_user->ID,'stm_dealer_image',true);
				
				//print_r( get_user_meta($none_dealer_user->ID,'place_categories',true));
                if(empty($userimg)){
				...
				include("content/place-listing.php");
