...
?>
    ...
    <div class="col-md-3 col-sm-12 classic-filter-row sidebar-sm-mg-bt <?php echo esc_attr($sidebar_pos['sidebar']); ?>" style="margin-top: -20px;">
        ...
        <!--Sidebar-->
		<form id="service_category_form"  method="get" action="/service-list/">
			...
			<div class="stm-accordion-inner">
					...
					<div class="row row-pad-top-24">
						...
						<div class="col-md-12 col-sm-12">
							<h5 class="pull-left">Listings Near Me</h5>
										...
										<div class="stm-accordion-inner">
											<div class="tree_main">
												<ul id="bs_main" class="main_ul">
													<?php 
													$api_result['country_name'];
													$currentregion = get_terms( array(
														'taxonomy' => 'region',
														'name'    => array( $api_result['country_name']),
														'hide_empty' => false,
													) );
													//print_r($currentregion);
														...
														$checked = '';
														if( !empty($_GET['region_category']) && in_array($currentregion->name, $_GET['region_category']) ){ 
...
<h3 style="padding: 10px 25px 10px;border-bottom: 1px solid #dddddd;margin-bottom: 0px;" class="advanceserch">Advanced Search </h5>
</div>

<?php 

$service_categories = get_terms(array('taxonomy' => 'service_category', 'hide_empty' => false, 'parent' => 0, 'exclude' => '21276'));
...
<div class="stm-accordion-single-unit stm-listing-directory-checkboxes stm-one_col stm-ajax-checkbox-instant">
						<a class="title collapsed" data-toggle="collapse" href="#accordion-dealerships-category-<?php echo $service_categorie->term_id; ?>" aria-expanded="false">
							<h5><div class="imginv-icon"><input type="checkbox" class="chk_filter_removeall chk_filter_condition" data-class="chk_all_filter_condition"  name="service_category[]"
                                value="<?php echo $service_categorie->term_id; ?>" <?php if( !empty($_GET['service_category']) && in_array($service_categorie->term_id, $_GET['service_category']) ){ echo " checked=checked "; } ?> /></div> <?php echo $service_categorie->name;?></h5>
                                <?php if($allservice){ ?>
							<span class="minus"></span>
						<?php } ?>
						</a>
						<?php if($allservice){ ?>
						<div class="stm-accordion-content">
							<div class="collapse content <?php echo $dealercategory2;?>" id="accordion-dealerships-category-<?php echo $service_categorie->term_id; ?>">
								<div class="stm-accordion-content-wrapper stm-accordion-content-padded">
									<div class="stm-accordion-inner">
...
<div class="stm-accordion-single-unit stm-listing-directory-checkboxes stm-one_col stm-ajax-checkbox-instant" style="display: none;">
						...
						<div class="stm-accordion-content">
							<div class="collapse in content <?php echo $dealerparts2;?>" id="accordion-dealerships-parts">
								<div class="stm-accordion-content-wrapper stm-accordion-content-padded">
									<div class="stm-accordion-inner">
...
<form  method="get" action="/service-list/" id="dealershipsresetofworld" class="dealershipsresetofworld" style="display: none;">
		...
		<br/>
			...
			<?php 
				//echo do_shortcode('[adning id="141271"]');
				 echo do_shortcode('[widget id="custom_html-8"]'); ?>
    ...
    <div class="stm-ajax-row">
        ...
        <div id="listings-result servicelist">
			...
			if(!isset($_GET['ca_location'])){
				...
				if(!empty($_GET['region_category']) && empty($_REQUEST['business_name'])){                
					$region_category = $_GET['region_category'];										
					foreach( $region_category as $cat ){				
						$meta_query_args[] = array(
							'key'     => 'billing_country',
							'value'   => get_country_name($cat),
							'compare' => 'LIKE'
						);
					}				
				}else{	
...
<script type="text/javascript">
	...
	console.log("sdfdfdsfdsfsd: "+locations.length);
	var origin;
...
//	}
    ...
    jQuery('.stm-accordion-single-unit a input[type="checkbox"]').click(function(e) {
    	e.stopPropagation();
