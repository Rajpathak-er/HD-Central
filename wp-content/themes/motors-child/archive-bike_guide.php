...
class User extends Authenticatable
{
    ...
    protected $appends = [
        'user_id', 'user_unique_id', 'is_notification', 'is_document_verified_formatted',
        'total_followers', 'total_followings', 'user_account_type_formatted', 'total_posts',
        'total_fav_users', 'total_bookmarks', 'is_subscription_enabled', 'share_link',
        'orders_count', 'tipped_amount', 'is_user_online', 'is_welcome_steps',
        'video_call_amount_formatted', 'verified_badge_file', 'total_video_call_requests',
        'total_post_payment', 'about_formatted', 'eyes_color_formatted', 'height_formatted',
        'weight_formatted', 'audio_call_amount_formatted', 'wallet_balance_formatted', 'is_stories'
    ];

    // Accessor for checking if user has stories
    ...
    // Accessor for unique user ID
    public function getUserUniqueIdAttribute() {
        return $this->unique_id;
    }

    // Accessor for email notification status
    ...
    // Accessor for subscription enabled status
    ...
    // Accessor for formatted wallet balance
    ...
    // Accessor for share link
    public function getShareLinkAttribute() {
        return \Setting::get('frontend_url') . $this->unique_id;
    }

    // Accessor for total followings
    ...
    // Accessor for total favorite users
    ...
    // Accessor for verified badge file
    public function getVerifiedBadgeFileAttribute() {
        return $this->is_verified_badge ? \Setting::get('verified_badge_file') : '';
    }

    // Accessor for total bookmarks
    ...
    // Relationship with user billing accounts
    public function userBillingAccounts() {
        return $this->hasMany(UserBillingAccount::class, 'user_id');
    }

    // Relationship with user documents
    public function userDocuments() {
        return $this->hasMany(UserDocument::class, 'user_id');
    }

    // Relationship with delivery addresses
    ...
    // Accessor for total post payment
    ...
    // Relationship with user referral code
    ...
    // Relationship with user products
    public function userProducts() {
        return $this->hasMany(UserProduct::class, 'user_id');
    }

    // Relationship with user session
    ...
    public function scopeApproved($query) {
        return $query->where('users.status', USER_APPROVED)->where('is_email_verified', USER_EMAIL_VERIFIED);
    }

    // Scope for document verified users
    public function scopeDocumentVerified($query) {
        return $query->where('users.is_document_verified', USER_DOCUMENT_APPROVED);
    }

    // Common response scope
    public function scopeCommonResponse($query) {
        return $query->select('users.id as user_id', 'users.unique_id as user_unique_id', 'users.*');
    }

    // Other response scope
    public function scopeOtherResponse($query) {
        return $query->select('users.id as user_id', 'users.unique_id as user_unique_id', 'users.*');
    }

    // Accessor for formatted video call amount
    ...
    // Accessor for formatted audio call amount
    ...
    // Accessor for total video call requests
    ...
    public static function boot() {
        ...
        static::deleting(function ($model) {
            ...
            foreach ($model->postPayments as $postPayment) {
            ...
            foreach ($model->fromUserSubscriptionPayments as $fromUserSubscriptionPayment) {
            ...
            foreach ($model->userProducts as $userProduct) {
                $userProduct->delete();
            }
    ...
    // Generates email verification code and expiry
