...
if ( !function_exists( 'stm_account_navigation' ) ) {  
    function stm_account_navigation()
    {
        $nav = array(
			...
			'my-harley-davidson' => array(
                'label' => esc_html__( 'My Harley – Davidson', 'motors' ),
                'url' => add_query_arg( array( 'page' => 'my-harley-davidson' ), stm_get_author_link( '' ) ),
                'icon' => 'fa fa-cog',
            ),
			'subscriptions' => array(
                'label' => esc_html__( 'My Subscription ', 'motors' ),
                'url' => add_query_arg( array( 'page' => 'subscriptions' ), stm_get_author_link( '' ) ),
                'icon' => 'fa fa-cog',  
            ),
			'become-dealer' => array(
                'label' => esc_html__( 'Become Dealer', 'motors' ),
                'url' => add_query_arg( array( 'page' => 'become-dealer' ), stm_get_author_link( '' ) ),
                'icon' => 'fa fa-cog',
            ),
			'become-service-provider' => array(
                'label' => esc_html__( 'Become Service Provider', 'motors' ),
                'url' => add_query_arg( array( 'page' => 'become-service-provider' ), stm_get_author_link( '' ) ),
                'icon' => 'fa fa-cog',
            ),
        );
        ...
        if ( in_array( 'subscriber', (array) $user->roles ) ) {
            unset($nav['become-dealer']);
            unset($nav['become-service-provider']);
...
if ( !function_exists( 'stm_account_current_page' ) ) {
    function stm_account_current_page()
    {
        ...
        if ( !empty( $_GET['account_settings'] ) and $_GET['account_settings'] ) {
		...
		if ( !empty( $_GET['become_service_dealer'] ) and $_GET['become_service_dealer'] ) {
            $page = 'become-service-provider';
        }	
        ...
        if ( $site_color_style != 'site_style_default' ) {
            ...
            if ( $layout == 'motorcycle' ) {
				...
				if(defined('STM_MM_DIR_NAME')) {
					...
					$custom_style_css .= $wp_filesystem->get_contents( get_template_directory() . '/assets/css/vc_template_styles/stm_mm_top_vehicles.css' );
					$custom_style_css .= $wp_filesystem->get_contents( get_template_directory() . '/assets/css/vc_template_styles/stm_mm_top_makes_tab.css' );
					$custom_style_css .= $wp_filesystem->get_contents( get_template_directory() . '/assets/css/vc_template_styles/stm_mm_top_categories.css' );
				}
                ...
                if ( $layout !== 'boats' ) {
                    ...
                    if(stm_is_listing_five()) {
                        $custom_style_css = $wp_filesystem->get_contents( get_template_directory() . '/assets/css/app-listing_five.css' );
                    ...
                    if(stm_is_rental_two()) {
						...
						$custom_style_css .= $wp_filesystem->get_contents( get_template_directory() . '/assets/css/app-rental_two.css' );
                    ...
                    if(stm_is_equipment()) {
						...
						$custom_style_css .= $wp_filesystem->get_contents( get_template_directory() . '/assets/css/app-equipment.css' );
...
if ( !function_exists( 'stm_get_stm_select_sorting_options' ) ) {
    ...
    {
        $sorts = array(
			'date_high' => esc_html__( 'Date: latest first', 'motors' ),
			'date_low' => esc_html__( 'Date: oldest first', 'motors' ),
			'price_low' => esc_html__( 'Price: ddlowest first', 'motors' ),
...
function stm_get_sort_options_html () {
	...
	foreach ($sortBy as $k => $val) {
		...
		$html .= '<option yyyyy value="' . $k . '" ' . $selected . '>' . $val . '</option>';
	}
